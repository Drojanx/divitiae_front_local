<template>
    <div id="item-activity-bar" >
        <Sidebar v-model:visible="visible" :class="'b-'+colorMode+' txt-'+colorMode" position="right">
        <template #container="{ closeCallback }" >
            <div class="flex flex-column h-full activity-bg" style="background: linear-gradient(to right, rgb(246, 247, 249) 0%, rgb(246, 247, 249) 40%, white 40%, white 100%);">
                <div class="flex align-items-center justify-content-between px-4 flex-shrink-0 bg-gray-1" style="height:8%">
                    <span class="inline-flex align-items-center gap-2">
                        <svg width="35" height="40" viewBox="0 0 35 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="..." fill="var(--primary-color-color)" />
                            <path d="..." fill="var(--text-color)" />
                        </svg>
                        <span :class="'txt-'+colorMode" class="font-semibold text-2xl">Item Activity</span>
                    </span>
                    <span>
                        <Button type="button" @click="closeCallback" icon="pi pi-times" rounded outlined class="h-2rem w-2rem"></Button>
                    </span>
                </div>
                <div style="height:92%"  class="container-fluid p-0 flex flex-column justify-content-end" @submit.prevent="" >
                    ...
                    <template v-if="!loadingItemLogs">
                        ...
                        <div class="align-self-end w-100">
                            <div id="log-box" class="pt-4 pb-3 bg-gray-1" v-if="!sendingNewLog">
        ...
        <div :class="'txt-'+colorMode">
            <span class="me-0" style="font-weight: bold">Activity</span>
            <Button icon="pi pi-bars" id="side-bar-button" @click="visible = true" class="pt-1" />
...
export default {
    ...
    ...mapGetters(['loadingItemLogs', 'stateItemLogs', 'stateApp', 'stateItemDetail', 'colorMode', 'environment', 'stateWorkspace', 'isAdminInCurrentEnv', 'user', 'isAdminInCurrentEnv'])
